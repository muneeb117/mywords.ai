...
class ApiError implements Exception {
  ...
  ApiError({this.code, required this.errorMsg});
  ...
  factory ApiError.fromDioException(DioException dioException) {
    ...
    if (dioException.response != null) {
      ...
      switch (dioException.response?.statusCode) {
        case 400:
          return ApiError(code: dioException.response?.statusCode, errorMsg: dioException.response?.data['error']);
        case 401:
          return ApiError(code: dioException.response?.statusCode, errorMsg: dioException.response?.data['error']);
        case 404:
          return ApiError(code: dioException.response?.statusCode, errorMsg: dioException.response?.data['error']);
        ...
        case 503:
          return ApiError(code: dioException.response?.statusCode, errorMsg: dioException.response?.data['error']);
        default:
          return ApiError(code: dioException.response?.statusCode ?? 0, errorMsg: dioException.response?.data['error']);
      }
    } else if (dioException.type == DioExceptionType.connectionError) {
      return ApiError(code: dioException.response?.statusCode, errorMsg: 'Check your connection and try again.');
    }
    ...
    /// Exceptions on timeout
      ...
      return ApiError(code: 0, errorMsg: 'Connection timeout, Please try again');
    }

    return ApiError(code: dioException.response?.statusCode ?? 0, errorMsg: dioException.message);
  }
